# NEXUS GTM — Business Requirements Document (BRD)

**Status:** Production · **Audience:** Executive, Sales leadership, RevOps, Finance · **Last updated:** 2026‑06‑17

---

## 1. Executive summary

Go‑to‑market teams waste the majority of their selling time on low‑value work: researching accounts, guessing who's in‑market, writing one‑off emails, copying data into the CRM, and chasing stale sequences. NEXUS GTM is an AI GTM intelligence platform that **detects buying signals, scores accounts against the customer's ICP, drafts grounded outreach, and keeps the CRM in sync** — while keeping a human in the loop for every send.
...
6. **Consolidation** — signals + enrichment + scoring + sequencing + approvals + CRM sync in one workbench.
...
- **BR7 — Fast, low‑risk adoption:** deployable to any domain in one command; usable in a deterministic pilot mode with no API keys; switch to live providers via configuration.
...
**In scope (delivered):** signal ingestion; ICP scoring with tunable + learned weights; inbox/triage with recovery; account/contact enrichment, discovery, lookalikes; AI research/messaging/Q&A/contact‑rec; orchestrated runs; approvals with AI redraft, mailbox selection, reject reasons; campaigns; cadences; multi‑mailbox SMTP sending; email verification; HubSpot bi‑directional sync; plays; lists; alerts; analytics/attribution; multi‑tenant RBAC; one‑command secure deploy.

**Out of scope (now):** additional CRMs beyond HubSpot (Salesforce interface exists), inbound reply handling/meeting booking, dialer/voice, SSO/SCIM, advanced forecasting.

---

## 8. Assumptions & dependencies

- Customers provide their own provider keys for production output: an LLM key (Anthropic/Groq/OpenAI‑compatible), a search key (Exa) for signals/discovery, an email‑verification endpoint (Reacher‑class), a CRM token (HubSpot private app), and SMTP app passwords for sending.
- A **deterministic stub mode** runs the full loop with no keys for pilots and testing.
- Single‑VM Docker deployment with managed Postgres/Redis as an option for scale.
